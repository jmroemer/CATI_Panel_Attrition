...
# MODIFIED:   2021-05-17                                                       #
# VERSION:    More Narrow version of plot against survey round; convenient for #
#             showing two graphs next to each other.                           #
...
##     its baseline mean. 
  ...
  ## Create a one-line long_name for this narrow plot version. 
  summary_all_percentage$waves.long_name2 <- summary_all_percentage$waves.long_name
  summary_all_percentage$waves.long_name2[summary_all_percentage$waves.long_name2 == "Baseline\n(Face to Face)"] <- "Baseline"
  ...
  plot.dems.name <- function(data1, ymin, ymax, ytitle){
    ...
    plot_dem <- 
      ...
      coord_cartesian(ylim = c(ymin, ymax), expand = FALSE, 
                      xlim = c(0.5, (length(unique(data1$wave_name))+1.3)*1.58)) +
  ...
  plot.dems.name.specialBL <- function(data1, ymin, ymax, ytitle){
    plot_dem_special <- plot.dems.name(data1[data1$waves.months_from_BL != 0, ], 
      ...
      scale_x_discrete(labels = data1$waves.long_name2)                          # Use survey round name as labels.
    return(plot_dem_special)
  }
  ...
  ggsave(paste0(output_graphs, "/Sample_composition_of_respondents_against_round_", 
                study_id, ".png"), width = 4, height = 4, dpi = 200)
