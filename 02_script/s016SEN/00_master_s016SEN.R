...
# ADAPTED TO: Listen to Senegal survey.                                        #
...
## B. User options -------------------------------------------------------------
    ...
    study_reference <- "Senegal"
    
## C. Set folder folder paths --------------------------------------------------
    # Set relative references root folder
    ...
    if(!exists("root")) {
      ...
      if (file.exists(here::here("02_script", study_id, 
        ...
        stop("File path not found. Please close R and start it by opening   
                     the file 00_master_s016SEN.R or provide the root folder   
...
## A. Load raw data ------------------------------------------------------------
    ...
    # OUTPUTS:  One dataframe per survey round, named following rXX with XX 
    #           being the number of months passed since baseline.  
    source(here::here(script, study_id, "01a_load_raw_data_s016SEN.R"))
    

## B. Create a unique household number (uhn)------------------------------------
    # PURPOSE:  Create a respondent ID to allow for merging.                   
    #           Create a dataframe containing relevant metadata of the survey 
    #           rounds. 
    # INPUTS:   Dataframes rXX, containing the raw data
    # OUTPUTS:  <dfs> , <waves>, <study_id> 
    
    source(here::here(script, study_id, "01b_create_IDs_and_metadata.R"))
...
## Section 2: Merge and format the data ########################################
    ...
    # INPUTS:   <dfs>, <waves>        
    # OUTPUT:   <data_wide>,<data_wide_formatted>

    source(here::here(script, study_id, "02a_merge_recode_s016SEN.R"))
...
## Section 3: Calculate Response Rates #########################################
    ...
    ## Alternative version: Plots response rates for s016SEN together with 
    ##                      s016TZ into one graph. 
    source(here::here(script, study_id, "03_response_rates_plotting_SEN_TZA.R"))
